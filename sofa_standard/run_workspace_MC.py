...
# --- Fix PYTHONPATH for SOFA ---
base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
paths_to_add = [
    os.path.join(base_dir, "mCR_simulator-master/python"),
    os.path.join(base_dir, "mCR_simulator-master/python/mcr_sim_prb")
]
for p in paths_to_add:
    if p not in sys.path:
        sys.path.append(p)

import mcr_simulator
import mcr_external_magnet
import mcr_instrument
import mcr_magnet
import mcr_controller_sofa
...
# [FINAL STABLE DVS] In STATIC mode, we only need the 110mm cantilever.
# No 450mm base required as there is no IRC sliding.

if MODE == 3:
    # 3-Magnets (90mm + 20mm transition = 110mm active)
    ...
    # Added 20mm transition to reach 110mm total
    segments.insert(0, {'length': 0.020, 'E': 500e6, 'num_elem': 2, 'is_magnet': False})
elif MODE == 1:
    # 1-Magnet active (Total 110mm active)
    segments.append({'length': 0.080, 'E': 500e6, 'num_elem': 8, 'is_magnet': False})
    segments.append({'length': 0.020, 'E': 250e6, 'num_elem': 2, 'is_magnet': False})
...
class MonteCarloController(Sofa.Core.Controller):
    def __init__(self, root_node, instrument, epm, total_samples, wait_steps, mode, *args, **kwargs):
        ...
        self.sample_idx = 0
        self.results = []
        # Output to centralized logs folder
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        self.csv_filename = os.path.join(base_dir, "logs_and_data/csv_results", f"workspace_results_{self.mode}mag.csv")
...
def createScene(root_node):
    ...
    # [STABLE STATIC] Switching to Static Cantilever Mode (No IRC)
    # This mode eliminates numerical instability during global translation.
    instrument = mcr_instrument.Instrument(
        ...
        use_navigation=False, # Disable IRC for maximum stability
        color=[.2, .8, 1., 1.] if MODE==3 else [1., .5, .2, 1.]
    )

    # instrument.IRC.xtip.value = [0.110] # No longer needed in static mode
