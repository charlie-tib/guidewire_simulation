...
if VESSEL == "synthetic":
    ...
    VESSEL_STL = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "mCR_simulator-master/mesh/anatomies/J2-Naviworks.stl")
...
class NavigationController(Sofa.Core.Controller):
    ...
    def __init__(self, root_node, instrument, epm, mag_controller, 
        ...
        # CSV output to centralized logs folder
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        output_dir = os.path.join(base_dir, "logs_and_data/csv_results")
        self.csv_file = os.path.join(output_dir, f"nav_results_{mode_str}_{vessel_str}_trial{trial_id}.csv")
        self.summary_file = os.path.join(output_dir, f"nav_summary_{self.mode}mag_{VESSEL}.csv")
...
def createScene(root_node):
    ...
    if VESSEL == "synthetic":
        ...
        # Recess by 540mm so tip starts at wp0. Rotate Z-local into X-world.
        T_start_sim = [waypoints[0][0] - 0.540, waypoints[0][1], waypoints[0][2], 0., 0.7071, 0., 0.7071]
