...
def plot_workspace_comparison():
    ...
    data_dir = os.path.join(base_dir, "logs_and_data/csv_results")
    for label, f in files.items():
        try:
            full_path = os.path.join(data_dir, f)
            if not os.path.exists(full_path):
                # Try relative if central not found
                full_path = f
            dfs[label] = pd.read_csv(full_path)
        except FileNotFoundError:
            print(f"File {f} not found in {data_dir}, skipping...")
    ...
    plt.tight_layout()
    plot_path = os.path.join(base_dir, "logs_and_data/plots/python_dvs_workspace_analysis.png")
    plt.savefig(plot_path)
    print(f"Saved {plot_path} with 4 groups")
